					  ...
					  const char *result_format, void *result, char *args_format, ...)
{
	...
	pmethod = PyObject_GetAttrString(pobject, const_cast<char *>(method));
	if (!pmethod)
	{
		PyErr_Clear();
			  ...
			  const char *result_format, void *result)
{
	...
	pvariable = PyObject_GetAttrString(pmodule, const_cast<char *>(variable));
	Py_DECREF(pmodule);
...
void InitPy()
{
	Py_SetPythonHome("E:\\documents\\python\\hello");
	Py_Initialize();
	...
	if (pPath)
	{
		...
		PyObjectRunMethod(pPath, "append", "", 0, "(s)", "E:\\documents\\python\\hello");
		InitCCallPy();
		SetData("hello world!");
